...
const EN_GUIDE_FOLK_STRINGS = {
  ...
  hardanger_fiddle: {
    ...
    buy: "Entry $1,700-$4,800, mid-tier $4,800 and up — there is no Korean distribution, so ordering from a Norwegian workshop or buying used from overseas is the default. They are handmade, so even the floor is high. If the budget stings, you can dip a toe in with a violin modified to carry sympathetic strings. Landmine: cheap violins with only Hardanger-style decoration painted on do circulate, so always confirm the sympathetic strings are really there. For makers, ask the HFAA community for recommendations before you commit.",
  ...
  nyckelharpa: {
    ...
    buy: "Entry $1,350-$4,100 — no Korean distribution, so ordering from a European workshop is the default. Swedish and German makers often have waiting lists, so ask early (for maker recommendations, international communities like the American Nyckelharpa Association are the safe bet). If you are handy, starting from a build kit (around $680) is a genuine route. Landmine: cheap decorative units have misaligned keys and simply play out of tune — confirm it is built to be played.",
  ...
  gayageum: {
    ...
    buy: "Entry (practice grade) $210-$680, sanjo and performance grade $1,050-$4,100 — the standard is a practice model from a gugak instrument shop such as Seoul Gugaksa, Hanullim Gugakgi or gugakgi.com. The gap between practice and sanjo grades (paulownia body, silk-string quality) is large, so start with a practice model or a culture-center rental, confirm your taste, then upgrade. Landmine: the $70-odd decor-friendly models on open marketplaces will not hold their bridges or their tuning. Check that it came from an actual gugak shop!",
  ...
  koto: {
    ...
    buy: "Entry $270-$820 — with almost no Korean distribution, buying used from Japan (Yahoo Auctions or Mercari plus a proxy service) is standard. Used koto are so common there that shipping (a 1.8m freight item!) can cost more than the instrument — a famous joke that happens to be true. Landmine: on used units, study the photos for string condition and body cracks, and budget for a restring, which is a specialist job. Tsume come in finger sizes, so check yours!",
  ...
  kora: {
    ...
    buy: "Entry $550-$1,700 — no Korean distribution. Buying direct from Gambian or Senegalese makers, or commissioning a European workshop, are the routes, with shipping taking a big share of the cost. Important tip: models with traditional leather tuning rings (konso) are miserable to tune, so buy one upgraded with guitar pegs. Landmine: tourist koras have thin gourd bodies that cannot hold tuning. Ask West African music communities for maker recommendations before you order.",
  ...
  kalimba: {
    ...
    buy: "Entry $10-$40 — available anywhere in Korea (open marketplaces, discount variety stores, music shops). For the standard 17-key C major model, $20-odd already sounds plenty good. Chinese specialists like Gecko and Hluru, plus domestic names such as Minor Sound, are the usual first buys. Landmine: some of the cheapest have sharp tine edges or buzzing, so read the reviews. Hollow-body (acoustic) and flat-board models sound different, so watch demo videos before you choose!",
  ...
  zhongruan: {
    ...
    buy: "Entry $100-$340 — direct from Taobao, with a beginner model around 1,000 yuan (about $140) as the standard. Big factory brands like Xinghai and Dunhuang are the no-fail choices. Landmine: rock-bottom no-name units can have uneven fret placement and play out of tune, so check seller reviews and real playing footage. Put picks and spare strings in the same cart!",
  ...
  sanxian: {
    ...
    buy: "Entry $140-$410 — direct from Taobao, with beginner models (small and medium sanxian) from big brands like Xinghai starting in the $140s. Landmine: snakeskin quality varies a lot, so pick a high-volume seller with good reviews, and as with erhu, confirm CITES export papers are included. Sizes split into large and small sanxian — the easier-to-handle small one is the sensible entry.",
  ...
  jinghu: {
    ...
    buy: "Entry $35-$170 — direct from Taobao, among the cheapest of the huqin family. The rock-bottom tens-of-thousands units vary wildly though, so aim above $70 with a major brand or a well-reviewed workshop. Landmine: CITES snakeskin paperwork checks are the same as for erhu, and a mute is essential equipment rather than optional (it really is loud). Specs split into xipi and erhuang types, so start with a general-purpose one.",
