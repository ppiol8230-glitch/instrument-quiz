...
const INST_WOODWIND_FOLK = [
  ...
  // ─────────────────────────────────────────────
  {
    ...
    funFacts: ["타이타닉 OST의 그 애절한 관악 소리 상당 부분이 휘슬 계열이에요", "아일랜드에선 초등학생 국민 악기"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    funFacts: ["1970년대 피니언 오버튼이 대량 보급한, 민속악기치고 아주 젊은 악기예요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "흙을 구워 만든 새 모양 폐관 악기. 리코더처럼 불면 바로 소리가 나고, 운지를 잡으면 음정이 그대로 나오는 구조라 음감 부담이 적어요. 한국에 학원·교재 인프라가 의외로 탄탄한 생활악기.",
    ...
    practical: {
      ...
      quickReward: 5,
      maintenance: 1,
      lessonDesc: "문화센터·평생학습관 단골 강좌. 국내 교재·인강 풍부 — 이 배치에서 국악기 다음으로 레슨 구하기 쉬워요.",
    },
    ...
    funFacts: ["젤다의 전설 '시간의 오카리나' 덕에 게임 팬 입문이 유독 많아요", "이탈리아어로 '작은 거위'라는 뜻"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    family: "민속 목관·다관 플루트",
    desc: "길이가 다른 관 여러 개를 묶어 관 하나당 한 음을 내는 악기. 병 입구를 불듯 관 끝에 입김을 스치는 방식. 루마니아·안데스 두 갈래 전통이 있고, 한국에선 게오르그 잠피르풍 무드음악으로 익숙해요.",

    sound: {
      tone: ["숨소리 섞임", "애수", "몽환", "부드러움"],
      toneDesc: "숨소리가 절반쯤 섞인 아련한 소리. '외로운 양치기'의 그 애수. 비브라토를 걸면 노래하는 듯 울어요.",
      volume: 3,
      sustain: 1,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "관 끝에 U자 홈(노치)을 파고 입김을 걸쳐 부는 안데스 세로 피리. 리코더 같은 취구가 없어 첫 소리부터 스스로 각을 잡아야 해요. 엘 콘도 파사의 그 바람 소리.",

    sound: {
      tone: ["숨소리 섞임", "애수", "거칢", "맑음"],
      toneDesc: "바람이 절벽을 스치는 듯한 거친 숨소리와 맑은 심지가 공존. 셈여림에 따라 표정이 극적으로 변해요.",
      volume: 3,
      sustain: 1,
    },
    ...
    funFacts: ["잉카 시대 이전부터 쓰인 수천 년 역사의 악기", "사이먼 앤 가펑클 'El Condor Pasa'로 세계에 알려졌어요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "일본 대나무 종적. 비스듬히 깎인 취구에 입김을 걸치는데, 각도 몇 도 차이로 소리가 나고 안 나고가 갈려요. 첫 소리까지 몇 주가 걸리는 것으로 악명 높은 대신, 숨소리·음정의 미세한 흔들림까지 전부 표현이 되는 깊이가 있어요.",
    ...
    funFacts: ["에도 시대 코무소(허무승)들이 삿갓을 쓰고 불며 탁발한 수행 도구였어요", "이름은 표준 길이 1척 8촌(약 54.5cm)에서 왔어요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    aka: ["danso", "短簫"],
    family: "국악 관악·종적",
    desc: "한국의 세로 피리. 학교 음악 시간의 그 악기지만, 제대로 불면 정악의 단아한 소리가 나와요. 국악 관악 입문의 표준 관문이자 최저가 국악기.",

    sound: {
      tone: ["맑음", "소박함", "단아함", "애수"],
      toneDesc: "가늘고 단정한 소리. 요성(농음)을 걸면 단번에 국악의 호흡이 돼요.",
      volume: 2,
      sustain: 1,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    aka: ["daegeum", "大笒", "젓대"],
    family: "국악 관악·횡적",
    desc: "청공(갈대 속막)이 떨리며 특유의 갈라지는 울림을 내는 한국의 대형 가로 피리. 국악 관악의 간판. 관이 길고 지공이 넓어 폐활량과 팔·손가락 스트레칭을 모두 요구해요.",
    ...
    tracks: [
      { id: "jeongak", label: "정악대금", genres: ["국악·사물놀이"], note: "궁중·풍류 음악. 관이 더 길고 호흡이 깊어요." },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    aka: ["piri", "觱篥", "향피리"],
    family: "국악 관악·더블리드",
    desc: "겹서(더블리드)를 입에 깊숙이 물고 부는 한국의 세로 피리. 작은 몸집에서 국악관현악 주선율을 이끄는 큰 소리가 나와요. 리드 악기 특유의 입술·호흡 압박과 서(리드) 관리가 본론.",

    sound: {
      tone: ["콧소리", "빵빵함", "애수", "거칢"],
      toneDesc: "콧소리 섞인 굵고 강한 소리. 여리게 불면 구슬프고, 세게 불면 관현악을 뚫어요. 서양 오보에보다 훨씬 육성에 가까움.",
      volume: 4,
      sustain: 2,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    family: "국악 관악·더블리드(숌 계열)",
    desc: "나팔 모양 동팔랑을 단 한국의 겹리드 악기. 사물놀이·풍물판을 뚫고 나오는 그 찢어지는 소리의 주인공. 야외 음악용으로 태어난 만큼 음량이 이 배치 최상급 — 연습 장소가 곧 진입 장벽이에요.",
    ...
    funFacts: ["이날치·씽씽 등 국악 크로스오버 무대의 하이라이트 담당", "옛 이름 '날라리'가 소리 그 자체를 말해줘요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    sound: {
      ...
      toneDesc: "울음을 삼킨 사람 목소리 같은 소리. 세상에서 가장 슬픈 악기라는 수식이 따라다녀요.",
      volume: 3,
      sustain: 3,
    },
    ...
    funFacts: ["유네스코 인류무형문화유산", "글래디에이터·왕좌의 게임 등에서 '슬픔 전담' 악기로 섭외돼요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "취구랄 것이 거의 없는 갈대 관을 비스듬히 대고 부는 중동·수피 전통 악기. 샤쿠하치와 함께 '첫 소리 지옥' 양대산맥이지만, 나면 그 즉시 기도 소리 같은 숨결이 나와요.",

    sound: {
      tone: ["숨소리 섞임", "애수", "신비", "먹먹함"],
      toneDesc: "숨이 절반, 소리가 절반. 사막 바람 같은 톤으로 수피 명상 음악의 중심. 루미의 시 '갈대의 노래'가 이 악기 이야기예요.",
      volume: 2,
      sustain: 2,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    family: "민속 목관·횡적(인도)",
    desc: "키 없는 인도 대나무 가로 피리. 지공을 반만 막는 미분음과 긴 글리산도로 인도 라가의 유연한 선율을 그려요. 명상적 저음의 대형 관이 본령.",

    sound: {
      tone: ["부드러움", "몽환", "숨소리 섞임", "노래하는 듯"],
      toneDesc: "무게 없는 부드러운 소리가 미끄러지듯 흘러요. 크리슈나 신의 악기라는 신화 그대로 관능적이고 명상적.",
      volume: 3,
      sustain: 2,
    },
    ...
    body: {
      ...
      breath: 2,
      weight: 1,
      weightDesc: "연주용 E베이스 반수리는 75cm급으로 길어요.",
    ...
    funFacts: ["하리프라사드 차우라시아가 이 소박한 목동 피리를 세계 콘서트홀 악기로 끌어올렸어요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    aka: ["dizi", "笛子", "중국 젓대"],
    family: "민속 목관·횡적(중국)",
    desc: "취구와 지공 사이 막공(膜孔)에 갈대 속막(디모)을 붙여 특유의 밝은 비음을 내는 중국 가로 피리. 대금의 청과 같은 원리지만 소리는 훨씬 가볍고 화려해요. 중국 민악의 간판 선율 악기.",
    ...
    funFacts: ["막공 유무가 디지와 서양 플루트의 결정적 차이 — 이 막 하나가 '중국 소리'를 만들어요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    family: "민속 목관·종적(중국)",
    desc: "디지의 화려함과 정반대에 있는 중국 세로 피리. 막 없이 어둡고 그윽한 소리를 내며, 고금(구친)과의 이중주로 문인 음악의 정취를 담당해요. 단소·퉁소와 같은 계보.",

    sound: {
      tone: ["낮고 깊음", "먹먹함", "명상적", "나무 울림"],
      toneDesc: "달빛 같은 어둑한 소리. 디지가 대낮이라면 샤오는 밤. 여백이 많은 음악에 어울려요.",
      volume: 2,
      sustain: 2,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "호리병(표주박)에 프리리드 관을 꽂은 윈난 소수민족 악기. 리드가 관 속에 내장돼 하모니카처럼 불면 바로 소리가 나고, 톤은 클라리넷을 닮은 매끈한 벨벳. 드론 관이 달려 혼자서도 지속음(드론) 반주가 깔려요.",

    sound: {
      tone: ["부드러움", "몽환", "콧소리", "따뜻함"],
      toneDesc: "기름 바른 듯 매끄러운 비단 소리. 중국 여행지 배경음악의 그 톤. 호불호는 갈리지만 귀에 즉시 감겨요.",
      volume: 2,
      sustain: 3,
    },
    ...
    funFacts: ["이름 그대로 '호리병(葫芦)+실(丝) 같은 소리'라는 뜻", "윈난 다이족의 구애 악기였어요"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "입으로 가죽 백을 채워 챈터+드론 3개를 동시에 울리는 스코틀랜드 군악 백파이프. 야외 행진용으로 설계된 음량(전투 신호용)이라 소음·폐활량 게이트가 전체 DB 최상위. 대신 입문은 조용한 프랙티스 챈터로 1년쯤 따로 해요.",

    sound: {
      tone: ["웅장함", "빵빵함", "위압적", "콧소리"],
      toneDesc: "드론 3개의 지속 저음 위에 챈터가 우는, 벽 같은 소리. 한 번 울리면 공간을 통째로 접수해요.",
      volume: 5,
      sustain: 5,
    },
    ...
    funFacts: ["1·2차 대전에서 실제 돌격 신호로 쓰였어요", "국제적으로 '악기이자 무기'라는 농담이 공식화된 유일한 악기"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    desc: "입 대신 팔꿈치 풀무로 바람을 넣는 아일랜드 백파이프. 입김이 아니라서 침·폐활량 게이트를 완전히 우회하는 유일한 백파이프 — 대신 그 자리를 양팔·무릎을 총동원하는 조작 복잡도가 차지해요. 앉아서 연주하며, 소리는 백파이프 중 가장 달콤해요.",

    sound: {
      tone: ["애수", "노래하는 듯", "부드러움", "화려함"],
      toneDesc: "하이랜드의 포효와 딴판인, 울면서 노래하는 소리. 챈터 끝을 무릎에 댔다 떼며 만드는 스타카토·레가토 전환과 2옥타브 음역이 백파이프계 유일의 표현력을 만들어요.",
      volume: 3,
      sustain: 5,
    },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    family: "민속 목관·백파이프(입김 충전)",
    desc: "스페인 북서부 갈리시아(켈트 문화권)의 백파이프. 하이랜드보다 작고 음량도 한 단계 낮으며, 축제 음악의 흥이 몸통이에요. 카를로스 누녜스로 대표되는 현대 갈리시아 씬은 백파이프를 록 페스티벌 무대에 올렸죠.",

    sound: {
      tone: ["빵빵함", "화려함", "콧소리", "파티"],
      toneDesc: "하이랜드보다 밝고 유연한 소리. 드론 위에서 챈터가 춤곡(무녜이라)을 튕기듯 굴려요.",
      volume: 4,
      sustain: 5,
    },
  ...
  // ─────────────────────────────────────────────
  // 21. 쑤오나 — 중국 태평소, 음량 최상급
  // ─────────────────────────────────────────────
  {
    id: "suona",
    name: "쑤오나",
    aka: ["suona", "唢呐", "중국 태평소"],
    family: "민속 목관·더블리드(숌 계열·중국)",
    desc: "겹리드에 금속 나팔 벨을 단 중국의 태평소. 결혼식·장례식·축제를 수백 년 뚫어 온 야외 악기라 음량이 태평소와 나란히 전체 최상급이에요. 새 울음까지 흉내 내는 화려한 기교의 세계가 본체.",

    sound: {
      tone: ["빵빵함", "거칢", "화려함", "콧소리"],
      toneDesc: "쇳소리 섞인 찢어질 듯한 광휘. 한 번 울리면 다른 악기가 다 묻힌다는 소리의 주인공 — 신명과 통곡을 한 악기로 오가요.",
      volume: 5,
      sustain: 2,
    },
    ...
    body: {
      ...
      embouchure: 3,
      breath: 3,
      breathDesc: "겹리드 압박+큰 호흡. 전통 연주는 순환호흡(끊김 없이 불기)까지 감.",
      weight: 1,
      weightDesc: "30~40cm대. 휴대 완벽.",
      posture: "부담 없는 자세. 서서 노는 악기",
      postureRisk: [],
      handSize: "무관",
    },
    ...
    cost: {
      entryKRW: [30000, 150000],
      entryDesc: "알리 직구 기준 학생용 세트 3~8만, 쓸만한 연주용 홍목 10~15만. 국내 재고 유통은 사실상 없음. 초(哨子, 갈대 리드)는 소모품 별도.",
      midKRW: [200000, 600000],
      running: "초(리드) 소모품 + 금속 부품 관리. 리드 수급은 직구 의존.",
    },
    practical: {
      portability: 5,
      noise: 5,
      apartmentOk: "실내 연습 자체가 불가 수준(태평소와 동급). 야외·방음 연습실 전제.",
      selfLearn: 2,
      lessonKR: 1,
      entryBarrier: 4,
      quickReward: 3,
      maintenance: 4,
      lessonDesc: "국내 쑤오나 전문 레슨은 사실상 없음(태평소·피리 전공자에게 곁다리로 배우는 정도). 중국어권 인강은 무한대.",
    },

    vibe: {
      tags: ["파티", "무대 장악", "위압적", "자유로운"],
      trendy: 2,
      rarityKR: 5,
      showOff: 5,
    },
    ...
    kinship: {
      easyFrom: ["태평소", "피리", "오보에"],
      sameTuning: [],
      leadsTo: ["태평소", "수르나이·주르나 계열 탐구"],
    },
    ensemble: ["중국 민악 합주", "취고수 행사 음악", "크로스오버 무대"],

    tracks: [
      { id: "folk", label: "민속·전통", genres: ["동아시아 전통"], note: "혼례·축제 음악과 새 울음 모사(백조조봉)의 화려한 전통 기교 트랙." },
      { id: "keyed", label: "키식(加键) 현대", genres: ["동아시아 전통", "영화·게임 OST"], note: "키를 단 개량 쑤오나. 중국 민악 오케스트라의 반음계·협주곡 트랙." },
    ],

    caveats: [
      "소음 문제는 태평소·하이랜드 백파이프와 나란히 전체 DB 최악권 — 연습 장소 확보가 실력보다 먼저",
      "겹리드 압박+호흡이라 입술·볼 근육이 만들어질 때까지 몇 달",
      "국내 레슨·유통 인프라가 사실상 0 — 직구와 중국어권 인강에 의존하는 독학 모험",
    ],
    funFacts: ["'쑤오나가 울리면 황금 만 냥'이라는 속담처럼, 중국 인터넷에선 '어떤 악기도 못 이기는 악기의 깡패'로 밈이 됐어요", "이름은 페르시아 겹리드 악기 수르나(surnā)에서 왔어요 — 태평소와 같은 실크로드 형제"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    aka: ["bawu", "巴乌", "파오"],
    family: "민속 목관·프리리드(중국 윈난)",
    desc: "대나무 가로 피리에 하모니카식 금속 프리리드를 박은 윈난 소수민족 악기. 후루쓰와 같은 리드 원리를 호리병 없이 관 하나에 담은 형제 악기로, 클라리넷 저음을 닮은 벨벳 톤이 매력이에요.",

    sound: {
      tone: ["부드러움", "따뜻함", "낮고 깊음", "콧소리"],
      toneDesc: "숨을 얹으면 스르르 미끄러지는 매끈한 저음. 후루쓰보다 한 톤 어둡고 사람 목소리에 가까워, '말하는 악기'라는 별명이 있어요.",
      volume: 2,
      sustain: 3,
    },
    ...
    body: {
      ...
      breath: 1,
      weight: 1,
      weightDesc: "30~60cm 관 하나. 휴대 완벽.",
      posture: "가로 피리 자세(관이 가벼워 부담 적음)",
      postureRisk: [],
      handSize: "무관",
    },
    ...
    cost: {
      entryKRW: [20000, 80000],
      entryDesc: "알리 직구 기준 2~5만이면 수지·대나무 입문기(G·F조). 국내 유통 거의 없음 — 직구가 표준 경로.",
      midKRW: [100000, 300000],
      running: "리드 내장형이라 소모품 개념 없음(리드 고장 시 수리·교체).",
    },
    practical: {
      ...
      quickReward: 4,
      maintenance: 1,
      lessonDesc: "국내 레슨 사실상 없음. 후루쓰와 마찬가지로 중국어권 인강+운지표 독학이 정규 과정.",
    },

    vibe: {
      tags: ["이국적", "소박한", "명상적", "서정적"],
      trendy: 1,
      rarityKR: 5,
      showOff: 2,
    },
    ...
    kinship: {
      easyFrom: ["후루쓰", "플루트", "하모니카"],
      sameTuning: [],
      leadsTo: ["후루쓰", "생황"],
    },
    ensemble: ["윈난 민속 앙상블", "뉴에이지·힐링 세션"],

    caveats: [
      "음역이 1옥타브 남짓으로 후루쓰와 나란히 최협소권 — 부를 수 있는 곡 상한이 뚜렷",
      "프리리드 공통 — 세게 불면 음정이 주저앉아 여린 호흡 유지가 요령",
      "리드가 관 속에 붙박이라 틀어진 개체를 사면 손쓸 방법이 없음(리뷰 확인 필수)",
    ],
    funFacts: ["윈난 하니족·이족의 악기로, 말 대신 마음을 전하는 구애의 악기였다는 전설이 따라다녀요", "중국 영화·드라마 OST에서 '애잔한 저음 피리'가 흐르면 높은 확률로 바우입니다"],
  },
  ...
  // ─────────────────────────────────────────────
  {
    ...
    name: "쉰",
    aka: ["xun", "埙", "중국 훈"],
    family: "민속 목관·폐관식 기명악기(베슬 플루트·중국)",
    desc: "달걀 모양으로 빚어 구운 중국의 흙피리. 7천 년 전 신석기 유물로 출토되는 최고(最古)급 악기이자 오카리나의 조상 격이에요. 다만 리코더식 취구가 없어 병 입구를 불듯 스스로 입김 각을 잡아야 하는 점이 오카리나와의 결정적 차이.",

    sound: {
      tone: ["낮고 깊음", "먹먹함", "신비", "나무 울림"],
      toneDesc: "흙 속에서 울리는 어둑하고 오래된 소리. 옛사람들이 '가을바람 소리'에 비유한 그 톤 — 화려함 대신 여운으로 승부해요.",
      volume: 2,
      sustain: 1,
    },
    ...
    body: {
      ...
      breath: 2,
      weight: 1,
      weightDesc: "손안에 들어오는 달걀·주먹 크기.",
      posture: "두 손으로 감싸 드는 편한 자세",
      postureRisk: [],
      handSize: "10공식은 두 손으로 감싸쥐는 구조라 대체로 무관",
    },

    cost: {
      entryKRW: [20000, 100000],
      entryDesc: "알리 직구 기준 입문용 10공(펜홀더식) 2~5만, 명장 수제 도자기는 10만 이상. 국내 유통 거의 없음 — 직구 기준.",
      midKRW: [100000, 400000],
      running: "소모품 없음. 도자기라 낙하 파손이 유일한 리스크.",
    },
    practical: {
      portability: 5,
      noise: 2,
      apartmentOk: "음량 작고 어두운 소리라 아파트 안전권.",
      selfLearn: 3,
      lessonKR: 1,
      entryBarrier: 3,
      quickReward: 2,
      maintenance: 1,
      lessonDesc: "국내 레슨 사실상 없음. 중국어권 인강이 교실 — 조용한 악기라 독학 친화적.",
    },

    vibe: {
      tags: ["명상적", "고풍스러운", "이국적", "서정적"],
      trendy: 1,
      rarityKR: 5,
      showOff: 2,
    },

    genres: { main: ["동아시아 전통"], sub: ["명상·사운드힐링", "영화·게임 OST"] },
    ...
    ensemble: ["중국 민악 소합주(훈·지 이중주)", "명상·힐링 세션"],

    caveats: [
      "오카리나처럼 생겼지만 리코더식 취구가 없음 — 병 입구 불듯 입김 각을 잡는 첫 소리 관문(며칠~2주)이 있어요",
      "음역이 좁고 레퍼토리가 느리고 어두운 곡 위주 — 신나는 곡을 원하면 방향이 다름",
      "도자기라 떨어뜨리면 그대로 유물(파편)이 됨",
    ],
    funFacts: ["《시경》의 '형이 훈을 불면 아우가 지(篪)를 분다'는 구절 이후 훈은 형제 우애의 상징이 됐어요", "신석기 유적에서 나온 7천 년 전 도기 훈이 지금도 박물관에서 소리를 냅니다"],
  },
